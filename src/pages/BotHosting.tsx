...
import { appOrderForm } from "../data/orderForms";
...
const tabMeta: { id: TabType; labelKey: string; icon: string; color: string; gradient: string; desc: string; specs: string[] }[] = [
  { id: "default", labelKey: "app_hosting.specs_title", icon: "fas fa-robot", color: "#a855f7", gradient: "from-purple-500 via-fuchsia-400 to-purple-600", desc: "app_hosting.desc", specs: ["Node.js", "NVMe SSD", "Auto Restart", "DDoS Protection"] },
];

const color = "#a855f7";

export const BotHosting: React.FC<BotHostingProps> = ({ onAddToCart }) => {
  const { t } = useTranslation();
  const { products, loading } = useProducts();
  const botPlans = products.filter((p) => p.category === "bot");
  ...
  const openWA = (plan: Plan) => {
    const msg = `Halo TinnzStore, saya tertarik dengan App Hosting:\n\n*${plan.name}*\nRAM: ${plan.ram}\nStorage: ${plan.storage}\nHarga: Rp ${plan.price_idr.toLocaleString("id-ID")}/bulan\n\nMohon info lebih lanjut.`;
  ...
  return (
    <div className="pt-20 pb-16 min-h-screen flex flex-col gap-10 max-w-7xl mx-auto px-4 md:px-8 relative">
      <div className="fixed pointer-events-none inset-0 overflow-hidden -z-10">
        ...
        <div className="absolute top-[35%] right-[12%] w-16 h-16 border border-fuchsia-500/10 rounded-lg rotate-45 animate-pulse" style={{ animationDuration: '5s' }} />
        <div className="absolute bottom-[25%] left-[20%] w-20 h-20 border border-pink-500/10 rounded-full animate-pulse" style={{ animationDuration: '6s' }} />
        ...
        <div className="absolute bottom-[40%] left-[5%] w-px h-32 bg-gradient-to-b from-transparent via-fuchsia-500/20 to-transparent animate-pulse" style={{ animationDuration: '4.5s' }} />
      </div>
      ...
      >
        ...
        <PageTitle text={t('app_hosting.title')} className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-2" />
        <p className="text-xs md:text-sm mt-3 max-w-xl text-[var(--text-secondary)] font-light leading-relaxed">
          {t('app_hosting.subtitle')}
        </p>
      </motion.div>
      ...
      >
        {[
          { label: t('app_hosting.banner_runtime'), value: "Node.js", icon: "fas fa-code", color: "#a855f7" },
          { label: t('app_hosting.banner_storage'), value: "NVMe", icon: "fas fa-hdd", color: "#d946ef" },
          { label: t('app_hosting.banner_restart'), value: "Auto Restart", icon: "fas fa-sync", color: "#ec4899" },
          { label: "99.9%", value: "Uptime", icon: "fas fa-shield-alt", color: "#a855f7" },
        ].map((stat, i) => (
          <motion.div
            key={i}
            initial={{ opacity: 0, y: 20 }}
        ...
        {tabMeta.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] opacity-60">({botPlans.length})</span>
            </motion.button>
          );
        })}
      </div>
      ...
      <div className="mb-6">
        <h2 className="text-base font-black uppercase tracking-tight text-[var(--text-primary)] mb-2">{t('app_hosting.choose_package')}</h2>
        <p className="text-[11px] md:text-sm text-[var(--text-secondary)] font-light leading-relaxed">{t('app_hosting.choose_desc')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredPlans.map((plan, i) => (
            <AppCard key={plan.id} plan={plan} color={color} index={i} onOrder={handleWhatsAppOrder} onWA={openWA} />
          ))}
        </div>
      )}
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        transition={{ duration: 0.6 }}
        className="relative overflow-hidden rounded-2xl border border-purple-500/20 mt-4"
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 md:p-8 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">{t('app_hosting.custom_title')}</h3>
              <p className="text-xs text-[var(--text-secondary)] font-light mt-0.5">{t('app_hosting.custom_desc')}</p>
            </div>
          </div>
          <a
            href="https://wa.me/6287844812351?text=Halo%20TinnzStore%2C%20saya%20tertarik%20dengan%20Custom%20Bot%20Hosting%20spesifikasi%20khusus."
            target="_blank"
      ...
      <OrderModal
        ...
        plan={orderPlan}
        titleKey="Form Pemesanan App Hosting"
        fields={appOrderForm.fields}
        newOrderMessage={appOrderForm.newOrderMessage}
        renewalMessage={appOrderForm.renewalMessage}
      />
    </div>
  );
};

const AppCard: React.FC<{ plan: Plan; color: string; index: number; onOrder: (plan: Plan) => void; onWA: (plan: Plan) => void }> = ({ plan, color, index, onOrder, onWA }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none" style={{ opacity: 0.12 }}>
        <img src="/bot.png" alt="" className="w-full h-full object-cover object-top" />
        ...
        <div className="space-y-2.5 mb-4">
          {plan.specs_list && plan.specs_list.length > 0 ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 text-[11px] font-mono">
                <div className="w-5 h-5 flex items-center justify-center rounded shrink-0" style={{ backgroundColor: `${color}12` }}>
                  <i className="fas fa-sync text-[8px]" style={{ color }} />
                </div>
                <span className="text-[var(--text-secondary)] truncate leading-tight">Auto Restart</span>
              </div>
            </>
          )}
