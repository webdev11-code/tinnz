...
import { hytaleOrderForm } from "../data/orderForms";
import { motion, AnimatePresence } from "motion/react";
import { PageTitle } from "../components/PageTitle";
import { RelatedServices } from "../components/RelatedServices";

interface HytaleHostingProps {
...
const tabMeta: { id: TabType; labelKey: string; icon: string; color: string; gradient: string; desc: string; specs: string[] }[] = [
  { id: "hytale", labelKey: "hytale_hosting.title", icon: "fas fa-gamepad", color: "#3b82f6", gradient: "from-blue-500 via-cyan-400 to-blue-600", desc: "hytale_hosting.desc", specs: ["NVMe Gen4", "DDoS Protection", "Backup Slot", "Port Allocation"] },
];

export const HytaleHosting: React.FC<HytaleHostingProps> = ({ onAddToCart }) => {
  const { t } = useTranslation();
  const { products, loading } = useProducts();
  const hytalePlans = products.filter((p) => p.category === "hytale");
  const [activeTab, setActiveTab] = useState<TabType>("hytale");
  ...
  const openWA = (plan: Plan) => {
    const msg = `Halo TinnzStore, saya tertarik dengan Hytale Hosting:\n\n*${plan.name}*\nRAM: ${plan.ram}\nStorage: ${plan.storage}\nHarga: Rp ${plan.price_idr.toLocaleString("id-ID")}/bulan\n\nMohon info lebih lanjut.`;
  ...
  return (
    <div className="pt-20 pb-16 min-h-screen flex flex-col gap-10 max-w-7xl mx-auto px-4 md:px-8 relative">
      <div className="fixed pointer-events-none inset-0 overflow-hidden -z-10">
        ...
        <div className="absolute bottom-[25%] left-[20%] w-20 h-20 border border-sky-500/10 rounded-full animate-pulse" style={{ animationDuration: '6s' }} />
      ...
      >
        ...
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-blue-500/30 text-[10px] font-mono uppercase tracking-[0.2em] text-blue-400 bg-blue-500/10 mb-4 backdrop-blur-sm">
          ...
          {t('hytale_hosting.badge')}
        </span>
        <PageTitle text={t('hytale_hosting.title')} className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-2" />
        <p className="text-xs md:text-sm mt-3 max-w-xl text-[var(--text-secondary)] font-light leading-relaxed">
          {t('hytale_hosting.subtitle')}
        </p>
      </motion.div>
      ...
      >
        {[
          ...
          { label: "Advance DDoS Security", value: "8 Tbit/s Protection", icon: "fas fa-shield-alt", color: "#14b8a6" },
        ...
        {tabMeta.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] opacity-60">({hytalePlans.length})</span>
            </motion.button>
          );
        })}
      </div>
      ...
      <div className="mb-6">
        <h2 className="text-base font-black uppercase tracking-tight text-[var(--text-primary)] mb-2">{t('hytale_hosting.choose_package')}</h2>
        <p className="text-[11px] md:text-sm text-[var(--text-secondary)] font-light leading-relaxed">{t('hytale_hosting.choose_desc')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {hytalePlans.map((plan, i) => (
            <HytaleCard key={plan.id} plan={plan} color={color} index={i} onOrder={handleWhatsAppOrder} onWA={openWA} />
          ))}
        </div>
      )}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 md:p-8 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">{t('hytale_hosting.custom_title')}</h3>
              <p className="text-xs text-[var(--text-secondary)] font-light mt-0.5">{t('hytale_hosting.custom_desc')}</p>
            </div>
          </div>
          <a
            href="https://wa.me/6287844812351?text=Halo%20TinnzStore%2C%20saya%20tertarik%20dengan%20Hytale%20Hosting%20spesifikasi%20khusus."
            target="_blank"
      ...
      <OrderModal
        ...
        plan={orderPlan}
        titleKey="Form Pemesanan Hytale Hosting"
        fields={hytaleOrderForm.fields}
        newOrderMessage={hytaleOrderForm.newOrderMessage}
        renewalMessage={hytaleOrderForm.renewalMessage}
      />
    </div>
  );
};

const HytaleCard: React.FC<{ plan: Plan; color: string; index: number; onOrder: (plan: Plan) => void; onWA: (plan: Plan) => void }> = ({ plan, color, index, onOrder, onWA }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none" style={{ opacity: 0.12 }}>
        <img src="/hytale.png" alt="" className="w-full h-full object-cover object-top" />
