...
import { sampOrderForm } from "../data/orderForms";
import { motion, AnimatePresence } from "motion/react";
import { PageTitle } from "../components/PageTitle";
import { RelatedServices } from "../components/RelatedServices";

interface SampHostingProps {
...
const tabMeta: { id: TabType; labelKey: string; icon: string; color: string; gradient: string; desc: string; specs: string[] }[] = [
  { id: "samp", labelKey: "samp_hosting.title", icon: "fas fa-car", color: "#10b981", gradient: "from-emerald-500 via-green-400 to-emerald-600", desc: "samp_hosting.desc", specs: ["NVMe SSD", "DDoS Protection", "Port Allocation", "Backup"] },
];

export const SampHosting: React.FC<SampHostingProps> = ({ onAddToCart }) => {
  const { t } = useTranslation();
  const { products, loading } = useProducts();
  const sampPlans = products.filter((p) => p.category === "samp");
  const [activeTab, setActiveTab] = useState<TabType>("samp");
  ...
  const openWA = (plan: Plan) => {
    const msg = `Halo TinnzStore, saya tertarik dengan SAMP Hosting:\n\n*${plan.name}*\nRAM: ${plan.ram}\nStorage: ${plan.storage}\nHarga: Rp ${plan.price_idr.toLocaleString("id-ID")}/bulan\n\nMohon info lebih lanjut.`;
  ...
  return (
    <div className="pt-20 pb-16 min-h-screen flex flex-col gap-10 max-w-7xl mx-auto px-4 md:px-8 relative">
      <div className="fixed pointer-events-none inset-0 overflow-hidden -z-10">
        <div className="absolute top-[15%] left-[8%] w-24 h-24 border border-emerald-500/10 rounded-full animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute top-[35%] right-[12%] w-16 h-16 border border-green-500/10 rounded-lg rotate-45 animate-pulse" style={{ animationDuration: '5s' }} />
        ...
        <div className="absolute top-[60%] right-[25%] w-12 h-12 border border-green-400/10 rounded-lg -rotate-12 animate-pulse" style={{ animationDuration: '3.5s' }} />
        <div className="absolute top-[10%] right-[40%] w-32 h-px bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent animate-pulse" style={{ animationDuration: '3s' }} />
        <div className="absolute bottom-[40%] left-[5%] w-px h-32 bg-gradient-to-b from-transparent via-green-500/20 to-transparent animate-pulse" style={{ animationDuration: '4.5s' }} />
      </div>
      ...
      >
        <div className="absolute -top-10 w-40 h-40 bg-emerald-500/10 rounded-full blur-[80px] animate-pulse" />
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-emerald-500/30 text-[10px] font-mono uppercase tracking-[0.2em] text-emerald-400 bg-emerald-500/10 mb-4 backdrop-blur-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
          {t('samp_hosting.badge')}
        </span>
        <PageTitle text={t('samp_hosting.title')} className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-2" />
        <p className="text-xs md:text-sm mt-3 max-w-xl text-[var(--text-secondary)] font-light leading-relaxed">
          {t('samp_hosting.subtitle')}
        </p>
      </motion.div>
      ...
      >
        {[
          { label: "Enterprise Grade Processors", value: "High Performance CPU", icon: "fas fa-microchip", color: "#10b981" },
          { label: "Ultra Fast Storage", value: "NVMe SSD", icon: "fas fa-hdd", color: "#059669" },
          { label: "Advance DDoS Security", value: "8 Tbit/s Protection", icon: "fas fa-shield-alt", color: "#34d399" },
        ...
        {tabMeta.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] opacity-60">({sampPlans.length})</span>
            </motion.button>
          );
        })}
      </div>
      ...
      <div className="mb-6">
        <h2 className="text-base font-black uppercase tracking-tight text-[var(--text-primary)] mb-2">{t('samp_hosting.choose_package')}</h2>
        <p className="text-[11px] md:text-sm text-[var(--text-secondary)] font-light leading-relaxed">{t('samp_hosting.choose_desc')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {sampPlans.map((plan, i) => (
            <SampCard key={plan.id} plan={plan} color={color} index={i} onOrder={handleWhatsAppOrder} onWA={openWA} />
          ))}
        </div>
      )}
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 md:p-8 relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-400">
              <i className="fas fa-car text-lg" />
            </div>
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">{t('samp_hosting.custom_title')}</h3>
              <p className="text-xs text-[var(--text-secondary)] font-light mt-0.5">{t('samp_hosting.custom_desc')}</p>
            </div>
          </div>
          <a
            href="https://wa.me/6287844812351?text=Halo%20TinnzStore%2C%20saya%20tertarik%20dengan%20SAMP%20Hosting%20spesifikasi%20khusus."
            target="_blank"
      ...
      <OrderModal
        ...
        plan={orderPlan}
        titleKey="Form Pemesanan SAMP Hosting"
        fields={sampOrderForm.fields}
        newOrderMessage={sampOrderForm.newOrderMessage}
        renewalMessage={sampOrderForm.renewalMessage}
      />
    </div>
  );
};

const SampCard: React.FC<{ plan: Plan; color: string; index: number; onOrder: (plan: Plan) => void; onWA: (plan: Plan) => void }> = ({ plan, color, index, onOrder, onWA }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none" style={{ opacity: 0.12 }}>
        <img src="/samp.png" alt="" className="w-full h-full object-cover object-top" />
