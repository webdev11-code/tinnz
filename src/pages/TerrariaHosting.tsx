...
import { terrariaOrderForm } from "../data/orderForms";
import { motion, AnimatePresence } from "motion/react";
import { PageTitle } from "../components/PageTitle";
import { RelatedServices } from "../components/RelatedServices";

interface TerrariaHostingProps {
...
type TabType = "terraria";

const tabMeta: { id: TabType; labelKey: string; icon: string; color: string; gradient: string; desc: string; specs: string[] }[] = [
  { id: "terraria", labelKey: "terraria_hosting.title", icon: "fas fa-puzzle-piece", color: "#a855f7", gradient: "from-purple-500 via-violet-400 to-purple-600", desc: "terraria_hosting.desc", specs: ["NVMe Storage", "DDoS Protection", "Backup Slot", "Port Allocation"] },
];

export const TerrariaHosting: React.FC<TerrariaHostingProps> = ({ onAddToCart }) => {
  const { t } = useTranslation();
  const { products, loading } = useProducts();
  const terrariaPlans = products.filter((p) => p.category === "terraria");
  const [activeTab, setActiveTab] = useState<TabType>("terraria");
  ...
  const openWA = (plan: Plan) => {
    const msg = `Halo TinnzStore, saya tertarik dengan Terraria Hosting:\n\n*${plan.name}*\nRAM: ${plan.ram}\nStorage: ${plan.storage}\nHarga: Rp ${plan.price_idr.toLocaleString("id-ID")}/bulan\n\nMohon info lebih lanjut.`;
    window.open(`https://wa.me/6287844812351?text=${encodeURIComponent(msg)}`, "_blank");
  };

  const features = [
    { icon: "fas fa-puzzle-piece", text: t('terraria_hosting.feature1') },
    { icon: "fas fa-chart-line", text: t('terraria_hosting.feature2') },
    { icon: "fas fa-cog", text: t('terraria_hosting.feature3') },
  ];
  ...
  return (
    <div className="pt-20 pb-16 min-h-screen flex flex-col gap-10 max-w-7xl mx-auto px-4 md:px-8 relative">
      <div className="fixed pointer-events-none inset-0 overflow-hidden -z-10">
        <div className="absolute top-[15%] left-[8%] w-24 h-24 border border-purple-500/10 rounded-full animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute top-[35%] right-[12%] w-16 h-16 border border-pink-500/10 rounded-lg rotate-45 animate-pulse" style={{ animationDuration: '5s' }} />
        <div className="absolute bottom-[25%] left-[20%] w-20 h-20 border border-fuchsia-500/10 rounded-full animate-pulse" style={{ animationDuration: '6s' }} />
        <div className="absolute top-[60%] right-[25%] w-12 h-12 border border-purple-400/10 rounded-lg -rotate-12 animate-pulse" style={{ animationDuration: '3.5s' }} />
        <div className="absolute top-[10%] right-[40%] w-32 h-px bg-gradient-to-r from-transparent via-purple-500/20 to-transparent animate-pulse" style={{ animationDuration: '3s' }} />
        <div className="absolute bottom-[40%] left-[5%] w-px h-32 bg-gradient-to-b from-transparent via-pink-500/20 to-transparent animate-pulse" style={{ animationDuration: '4.5s' }} />
      </div>
      ...
      >
        <div className="absolute -top-10 w-40 h-40 bg-purple-500/10 rounded-full blur-[80px] animate-pulse" />
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-purple-500/30 text-[10px] font-mono uppercase tracking-[0.2em] text-purple-400 bg-purple-500/10 mb-4 backdrop-blur-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" />
          {t('terraria_hosting.badge')}
        </span>
        <PageTitle text={t('terraria_hosting.title')} className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-2" />
        <p className="text-xs md:text-sm mt-3 max-w-xl text-[var(--text-secondary)] font-light leading-relaxed">
          {t('terraria_hosting.subtitle')}
        </p>
      </motion.div>
      ...
      >
        {[
          { label: "Enterprise Grade Processors", value: "High Performance CPU", icon: "fas fa-microchip", color: "#a855f7" },
          { label: "Ultra Fast Storage", value: "NVMe SSD", icon: "fas fa-hdd", color: "#d946ef" },
          { label: "Advance DDoS Security", value: "8 Tbit/s Protection", icon: "fas fa-shield-alt", color: "#ec4899" },
        ...
        {tabMeta.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] opacity-60">({terrariaPlans.length})</span>
            </motion.button>
          );
        })}
      </div>
      ...
      <div className="mb-6">
        <h2 className="text-base font-black uppercase tracking-tight text-[var(--text-primary)] mb-2">{t('terraria_hosting.choose_package')}</h2>
        <p className="text-[11px] md:text-sm text-[var(--text-secondary)] font-light leading-relaxed">{t('terraria_hosting.choose_desc')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {terrariaPlans.map((plan, i) => (
            <TerrariaCard key={plan.id} plan={plan} color={color} index={i} onOrder={handleWhatsAppOrder} onWA={openWA} />
          ))}
        </div>
      )}
      ...
      >
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-[var(--text-primary)]">{t('terraria_hosting.features_title')}</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {features.map((feat, idx) => (
            ...
            >
              <div className="absolute -top-10 -right-10 w-20 h-20 rounded-full blur-xl opacity-0 group-hover:opacity-40 transition-opacity duration-500" style={{ backgroundColor: color }} />
              <div className="flex items-start gap-4 relative z-10">
                <div className="w-10 h-10 flex items-center justify-center rounded-xl shrink-0" style={{ backgroundColor: `${color}15`, color }}>
                  <i className={feat.icon} />
                </div>
                <p className="text-xs text-[var(--text-secondary)] leading-relaxed">{feat.text}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 md:p-8 relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-purple-500/20 border border-purple-500/30 text-purple-400">
              <i className="fab fa-whatsapp text-lg" />
            </div>
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-tight">{t('terraria_hosting.features_title')}</h3>
              <p className="text-xs text-[var(--text-secondary)] font-light mt-0.5">{t('terraria_hosting.desc')}</p>
            </div>
          </div>
          <a
            href="https://wa.me/6287844812351?text=Halo%20TinnzStore%2C%20saya%20tertarik%20dengan%20Terraria%20Hosting%20spesifikasi%20khusus."
            target="_blank"
      ...
      <OrderModal
        ...
        plan={orderPlan}
        titleKey="Form Pemesanan Terraria Hosting"
        fields={terrariaOrderForm.fields}
        newOrderMessage={terrariaOrderForm.newOrderMessage}
        renewalMessage={terrariaOrderForm.renewalMessage}
      />
    </div>
  );
};

const TerrariaCard: React.FC<{ plan: Plan; color: string; index: number; onOrder: (plan: Plan) => void; onWA: (plan: Plan) => void }> = ({ plan, color, index, onOrder, onWA }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none" style={{ opacity: 0.12 }}>
        <img src="/terraria.jpg" alt="" className="w-full h-full object-cover object-top" />
