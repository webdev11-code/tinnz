...
interface PrivateHostingProps {
  onAddToCart: (plan: Plan) => void;
}

type TabType = "atomic" | "catalyst" | "spectrum";
...
const tabMeta: { id: TabType; labelKey: string; icon: string; color: string; gradient: string; desc: string; specs: string[] }[] = [
  { id: "atomic", labelKey: "private_hosting.tab_atomic", icon: "fas fa-atom", color: "#3B82F6", gradient: "from-blue-500 via-cyan-400 to-blue-600", desc: "private_hosting.desc_atomic", specs: ["DDR4 RAM", "NVMe Storage", "DDoS Protection"] },
  { id: "catalyst", labelKey: "private_hosting.tab_catalyst", icon: "fas fa-flask", color: "#10B981", gradient: "from-emerald-500 via-green-400 to-emerald-600", desc: "private_hosting.desc_catalyst", specs: ["DDR5 RAM", "NVMe Gen4", "DDoS Protection"] },
  { id: "spectrum", labelKey: "private_hosting.tab_spectrum", icon: "fas fa-chart-bar", color: "#a855f7", gradient: "from-purple-500 via-violet-400 to-purple-600", desc: "private_hosting.desc_spectrum", specs: ["High Performance", "Priority Support", "DDoS Protection"] },
];

export const PrivateHosting: React.FC<PrivateHostingProps> = ({ onAddToCart }) => {
  ...
  const atomicPlans = products.filter((p) => p.category === "private-atomic");
  const catalystPlans = products.filter((p) => p.category === "private-catalyst");
  const spectrumPlans = products.filter((p) => p.category === "private-spectrum");

  const getFilteredPlans = () => {
    switch (activeTab) {
      case "atomic": return atomicPlans;
      case "catalyst": return catalystPlans;
      case "spectrum": return spectrumPlans;
      default: return atomicPlans;
    }
  };
  ...
  const openWA = (plan: Plan) => {
    const msg = `Halo TinnzStore, saya tertarik dengan Private Hosting:\n\n*${plan.name}*\nRAM: ${plan.ram}\nStorage: ${plan.storage}\nHarga: Rp ${plan.price_idr.toLocaleString("id-ID")}/bulan\n\nMohon info lebih lanjut.`;
  ...
  return (
    <div className="pt-20 pb-16 min-h-screen flex flex-col gap-10 max-w-7xl mx-auto px-4 md:px-8 relative">
      <div className="fixed pointer-events-none inset-0 overflow-hidden -z-10">
        ...
        <div className="absolute top-[35%] right-[12%] w-16 h-16 border border-emerald-500/10 rounded-lg rotate-45 animate-pulse" style={{ animationDuration: '5s' }} />
      ...
      >
        ...
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-blue-500/30 text-[10px] font-mono uppercase tracking-[0.2em] text-blue-400 bg-blue-500/10 mb-4 backdrop-blur-sm">
          ...
          {t('private_hosting.badge')}
        </span>
        <PageTitle text={t('private_hosting.title')} className="text-3xl md:text-5xl font-black uppercase tracking-tight mt-2" />
        <p className="text-xs md:text-sm mt-3 max-w-xl text-[var(--text-secondary)] font-light leading-relaxed">
          {t('private_hosting.subtitle')}
        </p>
      </motion.div>
      ...
      >
        {[
          { label: "Enterprise Grade Processors", value: "High Performance CPU", icon: "fas fa-microchip", color: "#3B82F6" },
          { label: "Ultra Fast Storage", value: "NVMe SSD", icon: "fas fa-hdd", color: "#10B981" },
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {tabMeta.map((tab) => {
          ...
          const count = tab.id === "atomic" ? atomicPlans.length : tab.id === "catalyst" ? catalystPlans.length : spectrumPlans.length;
          return (
            <motion.button
              key={tab.id}
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6 relative z-10">
            ...
            <div className="flex-1">
              <h3 className="text-base font-black uppercase tracking-tight" style={{ color: currentTab.color }}>
                {t(`private_hosting.tab_${activeTab}` as any)}
              </h3>
      ...
      <div className="mb-6">
        <h2 className="text-base font-black uppercase tracking-tight text-[var(--text-primary)] mb-2">{t('private_hosting.choose_package')}</h2>
        <p className="text-[11px] md:text-sm text-[var(--text-secondary)] font-light leading-relaxed">{t('private_hosting.choose_desc')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredPlans.map((plan, i) => (
            <PrivateCard key={plan.id} plan={plan} color={currentTab.color} index={i} onOrder={handleWhatsAppOrder} onWA={openWA} />
          ))}
        </div>
      )}
...
const PrivateCard: React.FC<{ plan: Plan; color: string; index: number; onOrder: (plan: Plan) => void; onWA: (plan: Plan) => void }> = ({ plan, color, index, onOrder, onWA }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded-xl overflow-hidden pointer-events-none" style={{ opacity: 0.12 }}>
        <img src="/private.png" alt="" className="w-full h-full object-cover object-top" />
